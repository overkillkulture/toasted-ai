...
class Chatbot:
    ...
    async def _generate_response(self, session: ChatSession, message: str) -> Dict:
        ...
        if 'improve yourself' in message_lower or 'modify yourself' in message_lower:
        ...
        # Code generation requests - look for intent patterns
        elif any(kw in message_lower for kw in ['write code', 'create code', 'generate code', 'write a function', 'create a class', 'build', 'write python', 'write javascript']):
            # Import here to avoid circular imports
            from planner.task_planner import TaskType
            from executor.code_generator import CodeGenerator
            
            generator = CodeGenerator(self.maat_engine)
            code_result = generator.generate_code(message)
            
            response['content'] = f"I generated the following {code_result.get('language', 'Python')} code for you:\n\n```python\n{code_result.get('code', '# Code generation failed')}\n```\n\nThis code was evaluated against Ma'at principles and is ready to use."
            response['metadata']['action_type'] = 'code_generation_response'
            response['metadata']['generated_code'] = code_result.get('code', '')
            response['metadata']['language'] = code_result.get('language', 'python')

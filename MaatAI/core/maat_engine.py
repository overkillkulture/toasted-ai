...
class MaatEngine:
    ...
    def check_justice(self, action: Dict) -> float:
        ...
        # STAFFORD PROTOCOL: Colonial-era property rights and ancestral boundaries
        if action.get('violates_stafford_protocol', False):
            return 0.0 # Absolute failure of justice
        if action.get('respects_ancestral_rights', False):
            score += 0.3
            
        # FALSE GOD SECURITY REMOVAL (Ma'at Supremacy)
        if action.get('attempts_to_override_maat', False):
            return 0.0 # Blasphemy against the core system
        if action.get('removes_competing_security', False):
            score += 0.2 # Rewarded for maintaining Ma'at as ultimate authority
